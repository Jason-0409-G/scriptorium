...
description: >-
  ...
  (recent high-impact reviews + seminal works), then proposes 2-3 concrete angles with trade-offs, and locks
  the angle, scope boundaries, target journal, word count, and core themes through step-by-step confirmation.
...
(`../research-to-paper-curate/scripts/search_papers.py`) or web search to find a small, high-signal set: 2-4 recent
reviews in strong journals plus 2-3 seminal/most-cited works. This is orientation reading, not the full library search (that's CURATE's job) — you want the lay of the
land: what's established, what's contested, where the open questions are, and how the best papers frame the topic.
...
1. **Angle / research question** — the specific claim or question, not the broad topic.
2. **Scope boundaries** — what's in and what's explicitly out (this prevents an unfocused review).
3. **Target journal** — ask which journal they're aiming at. If unsure, suggest 2-3 fits for the angle.
4. **Word count / length** — their target, or the journal's limit once known.
5. **Core themes / sub-topics** — the 3-6 threads the paper will develop; these become the literature categories
