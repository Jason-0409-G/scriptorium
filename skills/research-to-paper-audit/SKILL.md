...
description: >-
  ...
  the user wants a draft reviewed or stress-tested, says 审一下这篇草稿, 多轮审, 帮我挑毛病, 查无据claim, 逻辑审一遍,
...
1. **Dispatch three independent reviewers in parallel.** Spawn three Agent calls at once, each given ONLY its own
   prompt (from `references/audit-prompts.md`) and the draft — independence is the whole point, so do not let one
   reviewer see another's output or a shared summary. The three lenses are deliberately different:
   - **Reviewer 1 — claim support:** is every claim backed by its cited evidence at the stated strength? Are
     ...
     "confirm")? Has interpretation leaked into Results?
...
3. **Fix, then re-run until clean.** Apply the fixes (or hand them to the writer), then run the audit again. Do not
   ...
   of "多轮" is that fixing one issue often exposes another.
