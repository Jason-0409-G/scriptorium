...
Give each reviewer ONLY its own prompt plus the draft. Independence is what makes three reviewers worth more than
one check repeated, so do not share a summary or let them see each other's output. After all three return, write
the Editor Synthesis below.
...
```
is consistent with < may/could reflect < appears to < suggests < is predicted to < indicates < supports
...
> (b) claims hedged too weakly OR too strongly for their evidence; (c) interpretation that has leaked into the
> Results section (Results should state what was observed, not what it means). Quote each problem sentence and say
...
> central thesis, or wander? Do the Results stay descriptive and the Discussion stay interpretive? Are there logical
...
- **Important** — interpretation in Results, structural gaps, weak hedges, over-broad citations.
...
them to the writer), then **re-run all three reviewers until a round returns with no blocking items**. Report the
synthesis to the user and let them decide every contested point — the audit informs; the user decides.
